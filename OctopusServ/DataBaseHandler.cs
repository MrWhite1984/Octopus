    ...
    class DataBaseHandler
    {
        ...
        public static Reply CreateDB(string name)
        {
            if (File.Exists(Path.Combine(Program.configuration.octopusFilesPath, name + ".octp")))
                return new Reply(ReplyType.Error, "База данных с таким именем уже существует");
            else
            {
                ...
                usingDb = path + ".octp";
                return new Reply(ReplyType.Success, $"База данных {name} создана");
            }
        }
        ...
        public static Reply RenameDB(string newName)
        {
            if (!File.Exists(Path.Combine(usingDb)))
                return new Reply(ReplyType.Error, "База данных с таким именем не существует");
            ...
            return new Reply(ReplyType.Success, $"Имя успушно изменено на {newName}");
        }
        ...
        public static Reply CreateDictionary(string name)
        {
            ...
            {
                zipArchive.Dispose();
                return new Reply(ReplyType.Error, "Словарь с таким именем уже существует в базе");
            }
            zipArchive.CreateEntry(name + ".octpd");
            zipArchive.Dispose();
            return new Reply(ReplyType.Success, $"Словарь {name} создан");
        }
        ...
        public static Reply GetAllDB()
        {
            ...
            if (names.Length == 0)
                return new Reply(ReplyType.Error, "Базы данных не найдены");
            string namesList = "";
            ...
            return new Reply(ReplyType.Success, namesList);
        }
        ...
        public static Reply GetAllDictionaryFromDB()
        {
            ...
            if (dictionaries.Count == 0)
                return new Reply(ReplyType.Error, "База данных не содержит словарей");
            string namesList = "";
        ...
        public static Reply AddDataToDictionary(string key, string value, string dictionaryName)
        {
            ...
            {
                zipArchive.Dispose();
                return new Reply(ReplyType.Error, "Словарь с таким именем не существует");
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    zipArchive.Dispose();
                    return new Reply(ReplyType.Error, $"Ключ {key} уже существует в словаре");
                }
            }
            ...
            streamWriter.Close();
            zipArchive.Dispose();
            return new Reply(ReplyType.Success, "Запись добавлена");
        }
        public static Reply GetAllDataFromDictionary(string dictionaryName)
        {
            ...
            streamReader.Close();
            zipArchive.Dispose();
            return new Reply(ReplyType.Data, dict);
        }
